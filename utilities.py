...
get some job done without leading to a deeper
...
def get_correctGreyCmap(name):
    ...
    which can lead to a loss of contrast
    Instead a LinearSegmentedColormap should work better
    """
...
def kNearestNeighbours(x,k,eps = 0):
    ...
    inputs:
    x   - an array with shape (N,m) where N is number of points and m is dimension (2 = x,y coord.)
...
def draw_periodic_lines(starting_point, line_direction, separation_vector,x_lim = [0,1],y_lim = [0,1],**kwargs):
